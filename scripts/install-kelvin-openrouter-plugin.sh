...
usage() {
  cat <<'USAGE'
Usage: scripts/install-kelvin-openrouter-plugin.sh [options]

Install Kelvin's first-party OpenRouter WASM model plugin from the plugin index.
...
VENDORED_TARBALL="$(ls -1 "${VENDOR_DIR}"/kelvin.openrouter-*.tar.gz 2>/dev/null | sort -V | tail -1 || true)"

if [[ -n "${VENDORED_TARBALL}" && -z "${INDEX_URL}" ]]; then
  echo "[install-kelvin-openrouter-plugin] installing from vendored tarball: ${VENDORED_TARBALL}"
  ...
  if [[ ! -f "${TRUST_POLICY_PATH}" ]]; then
    ...
    echo "[install-kelvin-openrouter-plugin] wrote default trust policy: ${TRUST_POLICY_PATH}"
  fi
  exit 0
fi
