...
if [[ ! -f "${TRUST_POLICY_PATH}" || "${REFRESH_TRUST_POLICY}" == "1" ]]; then
  ...
  if ! jq -e '.publishers[]? | select(.id == "kelvin")' "${TRUST_POLICY_PATH}" >/dev/null; then
    merged="$(mktemp)"
    jq -s '
    ...
    echo "Updated trust policy with Kelvin publisher key: ${TRUST_POLICY_PATH}"
  fi
fi

VERSION_DIR="${PLUGIN_HOME}/kelvin.cli/0.1.0"
if [[ -d "${VERSION_DIR}" && "${FORCE}" != "1" ]]; then
  echo "Kelvin CLI plugin already installed: ${VERSION_DIR}"
else
...
cat <<EOF
Kelvin CLI plugin installation complete.
