...
# Convenience wrapper around the common kelvin-plugin-dev.sh development commands.
...
test: build     ## Validate plugin manifest structure
	@\$(SCRIPTS)/kelvin-plugin-dev.sh test --manifest \$(MANIFEST)

pack: build     ## Create distributable .tar.gz package in dist/
	@\$(SCRIPTS)/kelvin-plugin-dev.sh pack --manifest \$(MANIFEST)

install: pack   ## Install plugin into the local Kelvin plugin home
	@\$(SCRIPTS)/kelvin-plugin-dev.sh install --package \$(PACKAGE)

smoke: build    ## Run a local smoke test
	@\$(SCRIPTS)/kelvin-plugin-dev.sh smoke --manifest \$(MANIFEST)
...
Generated by \`scripts/kelvin-plugin-dev.sh new --runtime wasm_model_v1\`.
...
usage() {
  cat <<'USAGE'
Usage: scripts/kelvin-plugin-dev.sh <command> [options]
...
cmd_list() {
  # Translate --plugin-home <dir> to KELVIN_PLUGIN_HOME env; pass remaining args through.
  local plugin_home="" extra=()
  while [[ $# -gt 0 ]]; do
    case "$1" in
...
new_usage() {
  cat <<'USAGE'
Usage: scripts/kelvin-plugin-dev.sh new [options]
...
install_usage() {
  cat <<'USAGE'
Usage: scripts/kelvin-plugin-dev.sh install --package <plugin-package.tar.gz> [options]
...
index_install_usage() {
  cat <<'USAGE'
Usage: scripts/kelvin-plugin-dev.sh index-install --plugin <id> [options]
...
test_usage() {
  cat <<'USAGE'
Usage: scripts/kelvin-plugin-dev.sh test --manifest <plugin.json> [options]
...
pack_usage() {
  cat <<'USAGE'
Usage: scripts/kelvin-plugin-dev.sh pack --manifest <plugin.json> [options]
...
verify_usage() {
  cat <<'USAGE'
Usage: scripts/kelvin-plugin-dev.sh verify [options]
...
smoke_usage() {
  cat <<'USAGE'
Usage: scripts/kelvin-plugin-dev.sh smoke --manifest <plugin.json> [options]
...
cmd_index_install() {
  ...
  # Translate args for `kelvin plugin install`:
  # --plugin <id>            → positional id
  # --plugin-home <dir>      → KELVIN_PLUGIN_HOME env
  # --trust-policy-path <p>  → KELVIN_TRUST_POLICY_PATH env
  # --version <v>            → --version <v>
  # --force                  → --force
  # --index-url <url>        → KELVIN_PLUGIN_INDEX_URL env
  # --registry-url / --min-quality-tier have no equivalent and are silently dropped.
  local plugin_id="" plugin_home="" trust_policy="" index_url="" install_args=()
  for ((i=0; i<${#args[@]}; i++)); do
    case "${args[$i]}" in
      --plugin)          plugin_id="${args[$((i+1))]}"; ((i++)) ;;
      --plugin-home)     plugin_home="${args[$((i+1))]}"; ((i++)) ;;
      --trust-policy-path) trust_policy="${args[$((i+1))]}"; ((i++)) ;;
      --index-url)       index_url="${args[$((i+1))]}"; ((i++)) ;;
      --version|--force) install_args+=("${args[$i]}") ;;
      --registry-url|--min-quality-tier) ((i++)) ;;  # no equivalent, drop
    esac
  done
