...
if ! ensure_rust_toolchain_path; then
  echo "[test-e2e] missing required commands: cargo/rustup" >&2
  exit 1
fi
...
echo "[test-e2e] running kelvin cli daemon lifecycle e2e suite"
cargo test -p kelvin-cli --test startup_e2e
