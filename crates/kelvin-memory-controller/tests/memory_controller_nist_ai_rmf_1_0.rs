...
fn govern_default_build_profile_remains_minimal_and_non_nvidia() {
    ...
    assert!(features.contains(&"provider_sqlite".to_string())); // THIS LINE CONTAINS CONSTANT(S)
    assert!(!features.contains(&"provider_vector_nvidia".to_string())); // THIS LINE CONTAINS CONSTANT(S)
}
...
async fn govern_issuer_and_audience_pinning_are_enforced() {
    ...
    let mut wrong_issuer = claims_for(MemoryOperation::Health, &next_id("jti")); // THIS LINE CONTAINS CONSTANT(S)
    wrong_issuer.iss = "wrong-issuer".to_string(); // THIS LINE CONTAINS CONSTANT(S)
    let err = controller
        .health(Request::new(HealthRequest {
            context: Some(context_for(&wrong_issuer, &next_id("req"))), // THIS LINE CONTAINS CONSTANT(S)
        }))
        .await
        .expect_err("issuer mismatch");
    assert_eq!(err.code(), Code::InvalidArgument);

    let mut wrong_audience = claims_for(MemoryOperation::Health, &next_id("jti")); // THIS LINE CONTAINS CONSTANT(S)
    wrong_audience.aud = "wrong-audience".to_string(); // THIS LINE CONTAINS CONSTANT(S)
    let err = controller
        .health(Request::new(HealthRequest {
            context: Some(context_for(&wrong_audience, &next_id("req"))), // THIS LINE CONTAINS CONSTANT(S)
        }))
        .await
...
async fn map_claim_and_request_scope_must_match() {
    ...
    context.tenant_id = "other-tenant".to_string(); // THIS LINE CONTAINS CONSTANT(S)
...
async fn map_module_manifest_validation_rejects_bad_shapes() {
    let controller = controller_with_module(sample_wasm()).await;
    let mut manifest = sample_manifest(vec!["provider_sqlite".to_string()]); // THIS LINE CONTAINS CONSTANT(S)
    manifest.capabilities.push("memory_read".to_string()); // THIS LINE CONTAINS CONSTANT(S)
    let err = controller
...
async fn measure_health_reports_enabled_features_and_loaded_modules() {
    ...
    assert!(health
        .enabled_features
        .contains(&"provider_sqlite".to_string())); // THIS LINE CONTAINS CONSTANT(S)
    assert!(health.loaded_modules.contains(&"memory.echo".to_string())); // THIS LINE CONTAINS CONSTANT(S)
}
...
async fn measure_not_found_errors_are_typed_for_unknown_modules() {
    ...
    claims.module_id = "memory.unknown".to_string(); // THIS LINE CONTAINS CONSTANT(S)
    let context = RequestContext {
        module_id: "memory.unknown".to_string(), // THIS LINE CONTAINS CONSTANT(S)
        ..context_for(&claims, &next_id("req")) // THIS LINE CONTAINS CONSTANT(S)
    };
    let err = controller
        .read(Request::new(ReadRequest {
            context: Some(context),
...
async fn measure_idempotent_retries_return_stable_responses() {
    let controller = controller_with_module(sample_wasm()).await;
    let request_id = next_id("req"); // THIS LINE CONTAINS CONSTANT(S)
    ...
    let first_resp = controller
        .upsert(Request::new(UpsertRequest {
            ...
            key: "k".to_string(), // THIS LINE CONTAINS CONSTANT(S)
            value: b"v1".to_vec(), // THIS LINE CONTAINS CONSTANT(S)
    ...
    let second_resp = controller
        .upsert(Request::new(UpsertRequest {
            ...
            key: "k".to_string(), // THIS LINE CONTAINS CONSTANT(S)
            value: b"v2".to_vec(), // THIS LINE CONTAINS CONSTANT(S)
            metadata: Default::default(),
        }))
        .await
...
async fn manage_replay_protection_blocks_reused_jti() {
    let controller = controller_with_module(sample_wasm()).await;
    let jti = next_id("jti-replay"); // THIS LINE CONTAINS CONSTANT(S)
    let first = claims_for(MemoryOperation::Upsert, &jti);
    controller
        .upsert(Request::new(UpsertRequest {
            context: Some(context_for(&first, &next_id("req"))), // THIS LINE CONTAINS CONSTANT(S)
            key: "k".to_string(), // THIS LINE CONTAINS CONSTANT(S)
    ...
    let err = controller
        .upsert(Request::new(UpsertRequest {
            context: Some(context_for(&second, &next_id("req"))), // THIS LINE CONTAINS CONSTANT(S)
            key: "k".to_string(), // THIS LINE CONTAINS CONSTANT(S)
            value: b"v".to_vec(), // THIS LINE CONTAINS CONSTANT(S)
            metadata: Default::default(),
        }))
...
async fn manage_request_limits_block_large_upserts() {
    ...
    claims.request_limits.max_bytes = 8; // THIS LINE CONTAINS CONSTANT(S)
    let err = controller
        .upsert(Request::new(UpsertRequest {
            context: Some(context_for(&claims, &next_id("req"))), // THIS LINE CONTAINS CONSTANT(S)
            key: "k".to_string(), // THIS LINE CONTAINS CONSTANT(S)
            value: vec![7_u8; 64], // THIS LINE CONTAINS CONSTANT(S)
            metadata: Default::default(),
        }))
        .await
